...
export class PostsService {
  constructor(
    @InjectModel('post') private postsModel: Model<any>,
  ...
  async findOne(id: string) {
    if (!isValidObjectId(id)) throw new BadRequestException('bad request');
    const user = await this.postsModel.findById(id).populate('user');
    return user;
  }

  async update(id: string, updatePostDto: UpdatePostDto) {
    const updateuser = await this.postsModel.findByIdAndUpdate(
      id,
      updatePostDto,
      { new: true },
    );
